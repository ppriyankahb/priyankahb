...
SC_MODULE(Memory) {
    ...
    void b_transport(tlm_generic_payload& payload, sc_time& delay) {
        ...
        if (payload.get_command() == TLM_WRITE_COMMAND) {
            ...
            int binary = customPayload->data;
            int bcd = ((binary / 10) << 4) | (binary % 10);

            cout << "Memory: Received WRITE request. Binary = " << binary << ", BCD = " << bcd << endl;
...
SC_MODULE(Initiator) {
    ...
    void sendData() {
        ...
        customPayload.data = 5; // Example 4-bit binary number
        ...
        cout << "Initiator: Sending WRITE request. Binary = " << customPayload.data << endl;
